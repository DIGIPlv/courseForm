...
const purchaseBtns = document.getElementsByClassName("purchase_white-box");
const courseNameInput = document.getElementById("course-name");
for (let i = 0; i< purchaseBtns.length; i++){
    purchaseBtns[i].addEventListener("click", ()=>{
        buyCourse(i)
    })
}

const payMethod = document.getElementById("payment-type"); //addition
payMethod.value = "Rēķins e-pastā"; //addition

window.onhashchange = function () {
  let hash = location.hash;
  if (hash === "#registrationForm") {
    hideAll();
    altSections[0].classList.remove("hidden");
  } else if (hash === "#paymentSuccessful") {
    hideAll();
...
function buyCourse(price) {
            payType.value =  typeOpt[price];
            priceFild.value = prices[price]
            priceWordsFild.value = priceWords[price];
            courseNameInput.value = courseNameTxt[price];
...
if (typeDropdown.value != "Privātpersona") {
    typeDropdown.value = "Privātpersona";
  }
function showPerson() {
...
function hideAll() {
  hideElements(normalSections);
  hideElements(altSections);
}
...
function showNormalState() {
  for (let i = 0; i < normalSections.length; i++) {
    normalSections[i].classList.remove("hidden");
  }
}